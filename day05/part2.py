...
def compute(s: str) -> int:
    ...
    for move in moves[1:]:
        ...
        # move items many at a time as per the instructions
        count = int(items['count'])
        values = []
        for n in range(count):
            values.append(stacks[int(items['src'])].pop())
        stacks[int(items['dest'])].extend(reversed(values))

    return ''.join(line[-1] for line in stacks.values() if line)
